...
# Mock data
mock_post_cache: List[SimpleNamespace] = []


def init_mock_posts(total: int = 30) -> List[SimpleNamespace]:
    ...
    for i in range(1, total + 1):
        ...
        comments = [
            {
                ...
                "tagged_users": [
                    {"id": 1, "nickname": "zizon_admin", "profile_image_url": "https://example.com/profile.jpg"}
                ],
            }
        ]
        ...
        post_dict = {
            ...
            "view_count": i * 5,
            "likes_count": i % 3,
            "attachments": [{"id": i, "file_url": "https://example.com/file.pdf", "file_name": "example.pdf"}],
            "images": [{"id": i, "image_url": "https://example.com/image.jpg", "image_name": "example.jpg"}],
...
#  캐시가 없으면 초기화
def mock_posts(total: int = 30) -> List[SimpleNamespace]:
...
class AdminPostListView(APIView):
    ...
    @extend_schema(
        ...
        parameters=[
            OpenApiParameter(
                name="page", type=OpenApiTypes.INT, location=OpenApiParameter.QUERY, description="페이지 번호"
            ),
            OpenApiParameter(
                name="size", type=OpenApiTypes.INT, location=OpenApiParameter.QUERY, description="페이지 크기"
            ),
            OpenApiParameter(
                name="is_visible", type=OpenApiTypes.BOOL, location=OpenApiParameter.QUERY, description="노출 여부"
            ),
            OpenApiParameter(
                name="category_id", type=OpenApiTypes.INT, location=OpenApiParameter.QUERY, description="카테고리 ID"
            ),
            OpenApiParameter(
                name="ordering",
                type=OpenApiTypes.STR,
                location=OpenApiParameter.QUERY,
                description="정렬 필드 (created_at, view_count, likes_count)",
            ),
        ],
    )
