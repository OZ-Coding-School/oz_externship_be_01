...
class CourseEnrollmentStatsView(APIView):
    ...
    @extend_schema(
        summary="과정별 수강 등록 통계 조회",
        responses=CourseEnrollmentStatsSerializer,
        tags=["course"]
    )
    def get(self, request: Request, course_id: int) -> Response:
        course = get_object_or_404(Course, id=course_id)
        enrollments = StudentEnrollmentRequest.objects.filter(
            generation__course_id=course_id
        ...
        generation_stats = enrollments.values('generation__id', 'generation__number').annotate(
...
class CourseEnrollmentListView(APIView):
    ...
    @extend_schema(
        summary="과정별 수강 등록 목록 조회",
        responses=CourseEnrollmentListSerializer,
        tags=["course"]
    )
    def get(self, request: Request, course_id: int) -> Response:
        course = get_object_or_404(Course, id=course_id)
        enrollments = StudentEnrollmentRequest.objects.filter(
            generation__course_id=course_id
        ).select_related('generation', 'user').order_by('-created_at')
