...
class TestDeploymentStatusResponseSerializer(serializers.ModelSerializer):
    message = serializers.SerializerMethodField()
    now = serializers.SerializerMethodField()

    class Meta:
        model = TestDeployment
        fields = ("message", "status", "now", "open_at", "close_at")

    # 상태/오픈시간 검증을 Serializer 내부에서 수행 (프로젝트 공통 구조 형식을 따름)
    def validate(self, data):

        test_deployment = self.instance
        now = timezone.now()

        # 배포 상태 체크
        if test_deployment.status != TestDeployment.TestStatus.ACTIVATED:
            raise serializers.ValidationError("해당 시험은 비활성화 상태입니다. 참가할 수 없습니다.")

        # 오픈 시간 체크
        if now < test_deployment.open_at:
            raise serializers.ValidationError("시험 오픈 시간이 아직 도래하지 않았습니다. 참가할 수 없습니다.")

        # [추가] 미션에 명시되진 않았지만 서비스 로직상 필요 / 마감 시간이 지나면 응시 불가능하도록 검증
        if now > test_deployment.close_at:
            raise serializers.ValidationError("시험 응시 시간이 마감되었습니다. 참가할 수 없습니다.")
